...
const MESSAGE_VALID_1 = 'the kinderlumper\'s gonna get ya, he\'s gonna get you good';
...
describe('testing correct functionality', () => {
  ...
  test('Message is not shown before timeSent', () => {
    ...
    message = getDmMessage(user1.token, dm1);
    expect(message.length).toStrictEqual(1);
    expect(message[0].message).toStrictEqual(MESSAGE_VALID);

    // check that the delayed message does show up after delay
  ...
  test('test that dm message can be deleted before it is sent', () => {
    let timeSent = getTimestamp() + standardDelay;
    ...
    // check the delayed message still doesn't show up
    message = getDmMessage(user1.token, dm1);
    expect(message.length).toStrictEqual(1);
    expect(message[0].message).toStrictEqual(MESSAGE_VALID);

    // delete the message
    let messageId = parseInt(BODY(laterMessage).messageId);
    //console.log(`Deleting message: ${messageId}`)
    removeMessage(user2.token, messageId);

    // check that the delayed message does not show up after delay
...
describe('testing error handling', () => {
  test('invalid dmId => BAD REQUEST 400', () => {
    ...
    // user2 sends message later with bad dmId
    const laterMessage = POST(messagesendLaterDmPATH, {
      token: user2.token,
      dmId: "5",
      message: MESSAGE_VALID_1,
  ...
  test('length of message not between 1 and 1000 characters (inclusive) => BAD REQUEST 400', () => {
    ...
    const laterMessage1 = POST(messagesendLaterDmPATH, {
      token: user2.token,
      dmId: dm1,
      message: '',
      timeSent: timeSent
    ...
    const laterMessage2 = POST(messagesendLaterDmPATH, {
      token: user2.token,
      dmId: dm1,
      message: longMessage,
      timeSent: timeSent
  ...
  test('User not member of Dm => FORBIDDEN 403', () => {
